...
public class VintageFilter implements Filter {
    ...
    public BufferedImage apply(BufferedImage image) {
        ...
        BufferedImage vintageImage = new BufferedImage(mirrored.getWidth(), mirrored.getHeight(), BufferedImage.TYPE_INT_RGB);
        for (int y = 0; y < mirrored.getHeight(); y++) {
            for (int x = 0; x < mirrored.getWidth(); x++) {
                ...
                int newR = (int) Math.min(255, Math.max(0, (r * 0.3) + (g * 0.59) + (b * 0.11) - 20));
                int newG = (int) Math.min(255, Math.max(0, (r * 0.25) + (g * 0.5) + (b * 0.25) - 15));
                int newB = (int) Math.min(255, Math.max(0, (r * 0.2) + (g * 0.4) + (b * 0.4) - 10));

                int newRgb = (newR << 16) | (newG << 8) | newB;
                vintageImage.setRGB(x, y, newRgb);
            }
        }
        return vintageImage;
    }
